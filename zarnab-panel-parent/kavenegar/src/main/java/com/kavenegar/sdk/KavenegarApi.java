...
public class KavenegarApi {
    ...
    private String createQuery(Object... params) {
        try {
...
//    }
    ...
    private JsonElement execute(String path, Object... params) throws BaseException {
        ...
        String body = createQuery(params);
        ...
        try {
            ...
            if (responseCode != 200) {
                throw new HttpException("Http request exception , code : " + responseCode, responseCode);
            }
            ...
            JsonObject json = JsonParser.parseString(entity).getAsJsonObject();

            JsonObject returnJson = json.get("return").getAsJsonObject();
            if (returnJson.get("status").getAsInt() != 200) {
                throw new ApiException(returnJson.get("message").getAsString(),
                        returnJson.get("status").getAsInt());
            }

            return json.get("entries");

        } catch (IOException e) {
            throw new HttpException("Http Request Exception: " + e.getMessage(), 0);
        }
    }
